...
env = gym.make('myoHandReachRandom-v0')
...
def generate_experience(actor, device='cpu'):
    NUM_EPISODES = 2000
    MAX_STEPS = 25
    inputs = torch.zeros(NUM_EPISODES, STATE_DIM)
    labels = torch.zeros(NUM_EPISODES, MAX_STEPS, STATE_DIM + 1)
    for episode in range(NUM_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            new_state, reward, _, _ = env.step(action.detach().cpu().numpy())
